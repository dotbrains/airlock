...
const ENV_API: [string, string, string][] = [
  ...
  [
    "AIRLOCK_API_TOKEN",
    "(none)",
    "Bearer token gating the dashboard + management API. Unset = unauthenticated."
  ],
...
export default function Docs() {
      ...
      <div className="container docs-layout">
        ...
        <main className="doc">
          <section id="overview" className="doc-section">
            ...
            <p>
              Airlock opens any link in a short-lived, containerized browser session — from a{" "}
              <strong>web dashboard</strong> or by <strong>right-clicking a link</strong>. It’s like
              a cloud-browser service, but it runs entirely on a machine you own: no cloud, no
              account, no data leaving the host.
            </p>
            <p>
              Each session is a disposable <strong>Kasm</strong> browser container on Docker,
              created with <code>AutoRemove</code> and no persistence volumes. A single Express API
              (<code>apps/api</code>) is the only component that talks to Docker; the dashboard (
              <code>apps/web</code>), the extension, and a cleanup worker (<code>apps/worker</code>)
              all reach it over HTTP. There is no database — session state lives in container
              labels.
            </p>
          </section>
          ...
          <section id="quickstart" className="doc-section">
            <h2>Quickstart</h2>
            <p>
              You need a running Docker engine and Bun 1.1+. Install from the repo root, then run
              the three local processes — or the whole stack from one image with{" "}
              <code>docker compose up</code>.
            </p>
            <Code>{USAGE}</Code>
            <p>
              Open <code>http://localhost:5173</code>, enter your <code>AIRLOCK_API_TOKEN</code> if
              one is set (blank for local dev), then launch a session.
            </p>
          </section>
          ...
          <section id="entries" className="doc-section">
            <h2>Two ways in</h2>
            <p>
              Both the dashboard and the extension drive the same session API and hand back a link
              to a disposable browser.
            </p>
          ...
          <section id="api" className="doc-section">
            <h2>API reference</h2>
            <p>
              When <code>AIRLOCK_API_TOKEN</code> is set, the management API requires{" "}
              <code>Authorization: Bearer &lt;token&gt;</code>. Health probes and the{" "}
              <code>/s/:id</code> capability link are auth-exempt by design;{" "}
              <code>/api/internal/prune</code> uses a separate internal token.
            </p>
  ...
  "ttlSeconds": 1800
            ...
            <p className="muted">
              The 201 response returns <code>sessionId</code>, <code>browserUrl</code>, a
              per-session <code>vncPassword</code>, <code>expiresAt</code>, and the public{" "}
              <code>sessionUrl</code>. Container internals never cross the public seam.
            </p>
          </section>
          ...
          <section id="browsers" className="doc-section">
            <h2>Browsers</h2>
            <p>
              Each browser kind maps to a Kasm container image, configurable via the{" "}
              <code>AIRLOCK_IMAGE_*</code> variables. Docker pulls on first launch;{" "}
              <code>POST /api/images/pull</code> warms them all.
            </p>
            <div className="doc-table-wrap">
              <table className="doc-table">
                <thead>
          ...
          <section id="configuration" className="doc-section">
            <h2>Configuration</h2>
            <p>
              Both apps load env via a shared helper that walks up for a <code>.env</code> file (or
              honors <code>AIRLOCK_ENV_FILE</code>). Every variable has a default; the full table is
              in the repo.
            </p>
            <h3>API (selected)</h3>
            <div className="doc-table-wrap">
            ...
            <p className="muted">
              <code>AIRLOCK_INTERNAL_TOKEN</code> must agree across the API and worker — the API
              gates <code>/api/internal/prune</code> with it and the worker sends it as the{" "}
              <code>x-airlock-internal-token</code> header.
            </p>
          </section>
          ...
          <section id="security" className="doc-section">
            <h2>Security</h2>
            <p>
              The control plane is a single Express process that is the <strong>only</strong>{" "}
              component allowed to talk to Docker. A mounted <code>/var/run/docker.sock</code> is{" "}
              <strong>root-equivalent on the host</strong> — keep the API behind the bearer token
              and a TLS-terminating proxy whenever it is reachable beyond localhost.
            </p>
            <p>
              Sessions run with <code>AutoRemove</code>, no persistence volumes, a per-session VNC
              password, <code>no-new-privileges</code>, and memory / CPU / PID caps. By default they
              attach to a dedicated ICC-off bridge. When <code>AIRLOCK_API_TOKEN</code> is unset and
              the API is bound beyond loopback, it logs a loud startup warning.
            </p>
            <p>
              <a
                className="btn"
          ...
          <section id="deployment" className="doc-section">
            ...
            <p>
              ...
              <code>deploy/</code> builds the same root image and satisfies one contract.
              Host-socket adapters (Docker Compose, VM) bind-mount the socket; remote-engine
              adapters (Kubernetes, Fly, Render, Railway) point <code>AIRLOCK_DOCKER_HOST</code> at
              a TLS-protected engine.
            </p>
            <p>
              <a
                className="btn"
